...
## Auditoria honesta — reconstrução core

No live, a área Record estava comprimida numa coluna de cerca de 218–247 px, deixando o formulário de gravação difícil de usar; foi corrigida para uma grelha larga de setup + sessões. O click real em `Começar a gravar` devolveu `NotFoundError` porque o ambiente de browser não expõe microfone; a mensagem foi tornada accionável, distinguindo ausência de dispositivo/permissão e indicando importação local via Sons → My Sounds. O click real numa tecla C reproduziu áudio local (`C4 · áudio local reproduzido`) e `＋ Timeline` criou `Piano · C materializado na timeline`, confirmando que o piano não estava morto, mas a experiência não comunicava bem o estado. Foi removido um listener duplicado de `[data-studio-area]`, que podia executar duas transições no mesmo clique.
